...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Handlers for manual BL saving (delegated): saves to backend `manual_bl` and updates local cache/UI
  (function attachManualBlHandlers() {
    const t = document.getElementById('table-body');
    if (!t) return;

    t.addEventListener('click', async (ev) => {
      const btn = ev.target.closest && ev.target.closest('.save-manual-bl');
      if (!btn) return;
      const reqId = btn.dataset.request;
      const tr = btn.closest('tr');
      const input = tr ? tr.querySelector('.manual-bl-input') : null;
      const val = input ? input.value.trim() : '';
      if (!val) { alert('Veuillez saisir le numéro BL'); return; }
      btn.disabled = true;
      try {
        // try persist to backend; ignore errors but log them
        await api.patch(`/requests/${reqId}`, { manual_bl: val });
      } catch (err) {
        console.error('manual BL save API error', err);
        alert('Erreur lors de l\'enregistrement du BL (vérifiez la connexion)');
      }
      // attempt to transition status to PROCESSING (best-effort)
      try {
        await api.post('/requests/transition', { requestId: reqId, to: 'PROCESSING' });
      } catch (tErr) {
        // log but don't block UI if transition fails
        console.warn('Failed to transition request to PROCESSING', tErr);
      }
      // update local cache and UI optimistically
      const idx = requests.findIndex(r => String(r.request_id || r.id) === String(reqId));
      ...
      if (tr) {
        ...
        try {
          ...
          if (statusCell) {
            statusCell.textContent = translateStatus('PROCESSING');
    ...
    // start manual BL flow: show i18n alert then reveal input+save
    t.addEventListener('click', (ev) => {
      const start = ev.target.closest && ev.target.closest('.start-manual-bl');
      if (!start) return;
      const reqId = start.dataset.request;
      try {
        const message = (window.i18n && typeof window.i18n.t === 'function') ? window.i18n.t('manual_bl_not_found_alert') : 'Please enter BL manually.';
        alert(message);
      } catch (e) {
        alert('Please enter BL manually.');
      }
      // replace button with input + save
      const tr = start.closest('tr');
      const rid = escapeHtml(String(reqId || ''));
      const html = `<input class="manual-bl-input" data-request="${rid}" placeholder="Saisir BL" style="padding:6px;border:1px solid #ddd;border-radius:4px;width:160px;"> <button class="save-manual-bl" data-request="${rid}" style="margin-left:6px;padding:6px 8px;border-radius:4px;border:1px solid #ccc;background:#f3f4f6;cursor:pointer;">Enregistrer</button>`;
      if (tr) {
        const firstTd = tr.querySelector('td');
        if (firstTd) firstTd.innerHTML = html;
        const input = tr.querySelector('.manual-bl-input');
        if (input) input.focus();
      }
    });

    t.addEventListener('keydown', (ev) => {
      if (ev.target && ev.target.classList && ev.target.classList.contains('manual-bl-input') && ev.key === 'Enter') {
        const row = ev.target.closest('tr');
        const btn = row ? row.querySelector('.save-manual-bl') : null;
        if (btn) btn.click();
      }
    });
  })();
...
// Format amount to always show dollar sign prefix

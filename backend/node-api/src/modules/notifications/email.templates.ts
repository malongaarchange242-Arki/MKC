...
`
  ...
  REQUEST_STATUS_CHANGED: (lang: 'fr' | 'en', input: EmailTemplateInput) =>
    brandedLayout(
      lang === 'en' ? 'Request status update' : 'Mise à jour de la demande',
      `
<p>${greeting(lang, input.prenom)},</p>
${
  lang === 'en'
...
────────────────────────────</pre>
<p style="margin:0">• Client : ${escapeHtml((input as any).client_name || '—')}</p>
<p style="margin:0">• Email client : ${escapeHtml((input as any).client_email || '—')}</p>
<p style="margin:0">• Référence : ${escapeHtml(input.requestRef || (input as any).entityId || '—')}</p>
<p style="margin:0">• Nouveau statut : ${escapeHtml((input as any).status || '—')}</p>
<p style="margin:0">• Date : ${escapeHtml((input as any).date || new Date().toISOString())}</p>
...
<p> Accès admin :<br/>${escapeHtml((input as any).admin_dashboard_url || process.env.ADMIN_DASHBOARD_URL || '—')}</p>

        ...
        try {
          ...
          try {
            ...
            if (blValue) {
              ...
              // No BL found: ensure request moves to AWAITING_DOCUMENTS and generate automatic BL reference
              try {
                const { data: reqRow, error: reqErr } = await supabase
                  .from('requests')
                  .select('id, status, manual_bl, user_id')
                  .eq('id', requestId)
                  .single();

                if (!reqErr && reqRow && reqRow.status === 'CREATED') {
                  // Generate automatic BL reference if not already set
                  let blRef = reqRow.manual_bl;
                  const wasAutoGenerated = !blRef;
                  
                  if (!blRef) {
                    try {
                      blRef = await AdminService.generateBlReference();
                      logger.info('Generated automatic BL reference after OCR failure', { requestId, blRef });
                    } catch (genErr) {
                      logger.error('Failed to generate automatic BL reference', { requestId, err: genErr });
                      // Continue without auto-generated reference; admin can manually regenerate later
                    }
                  }

                  // Update request with status and BL reference if generated
                  const updatePayload: any = { status: 'PROCESSING' };
                  if (blRef) {
                    updatePayload.manual_bl = blRef;
                    updatePayload.bl_number = blRef;
                  }
                  ...
                  if (updErr) {
                    logger.error('Failed to set PROCESSING or BL reference', updErr);
                  } else if (blRef && wasAutoGenerated) {
                    logger.info('Automatic BL reference saved', { requestId, blRef });
                    
                    // Send notification to client about auto-generated BL
                    try {
                      // Use REQUEST_STATUS_CHANGED (FR) so emails use the existing localized status template
                      await NotificationsService.send({
                        userId: reqRow.user_id,
                        type: 'REQUEST_STATUS_CHANGED',
                        title: 'Mise à jour du statut',
                        message: `Votre référence BL a été automatiquement générée : ${blRef}`,
                        request_id: requestId,
                        requestRef: blRef,
                        channels: ['in_app', 'email'],
                        language: 'fr',
                        metadata: { bl_reference: blRef }
                      });
                      logger.info('Sent status-update notification for auto-generated BL (in_app+email)', { requestId, userId: reqRow.user_id });
                    } catch (notifErr) {
                      logger.error('Failed to send status-update notification for auto-generated BL', { requestId, err: notifErr });

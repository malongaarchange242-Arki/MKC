...
class DocumentsService {
    ...
    static buildDocumentFilePath(requestId, uniqueFileName, subfolder) {
        if (subfolder)
            return `${requestId}/${subfolder}/${uniqueFileName}`;
    ...
    // ===============================
    static async checkRequestAccess(requestId, userId, userRole) {
    ...
    // ===============================
    static async uploadDocument(input, userRole) {
        // 1. On récupère docType depuis l'input
        const { requestId, file, uploadedBy, docType } = input;
        logger_1.logger.info('Uploading document', { requestId, fileName: file.originalname, docType });
        // Valider le fichier
        ...
        const { data: request } = await supabaseAdmin_1.supabaseAdmin
            .from('requests')
            .select('status')
            .eq('id', requestId)
            .single();
        ...
        // Les clients ne peuvent uploader que si la demande est CREATED ou AWAITING_DOCUMENTS
        if (userRole === 'CLIENT' && !['CREATED', 'AWAITING_DOCUMENTS'].includes(request.status)) {
        ...
        logger_1.logger.info('Storage path used', { storagePath, bucket: STORAGE_BUCKET });
        // Verify file exists in storage (single immediate check)
        const { data: listData, error: listError } = await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).list(requestId);
        if (!listData || !Array.isArray(listData) || !listData.find(f => f.name === uniqueFileName)) {
            // Attempt cleanup and surface clear error
            await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).remove([storedPath]).catch(() => null);
            logger_1.logger.error('Storage upload verification failed', { storagePath, uploadData, listError });
            throw new Error('Storage upload verification failed');
        }
        // Récupérer la version actuelle du document (si existe)
        const { data: existingDocs } = await supabaseAdmin_1.supabaseAdmin
            .from('documents')
            .select('version')
        ...
        const { data: documentData, error: dbError } = await supabaseAdmin_1.supabaseAdmin
            .from('documents')
            .insert({
            id: documentId,
            ...
            type: docType // 👈 CRITIQUE : On enregistre le type (ex: "bill_of_lading")
        })
            .select()
            .single();
        if (dbError || !documentData) {
            // Nettoyer le fichier uploadé en cas d'erreur DB
            await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).remove([storedPath]);
    ...
    static async getDocumentById(documentId, userId, userRole) {
        logger_1.logger.info('Fetching document', { documentId });
        const { data: document, error } = await supabaseAdmin_1.supabaseAdmin
            .from('documents')
            .select('*')
            .eq('id', documentId)
            .single();
    ...
    static async downloadDocument(documentId, userId, userRole) {
        ...
        const { data: fileData, error: downloadError } = await supabaseAdmin_1.supabaseAdmin.storage
            .from(STORAGE_BUCKET)
            .download(document.file_path);
        if (downloadError || !fileData) {
            logger_1.logger.error('Failed to download file from storage', { downloadError });
    ...
    // ===============================
    static async createFinalDocumentFromExisting(existing, adminId, type = null) {
        logger_1.logger.info('Creating final document entry from existing file', { existing: existing.id, adminId });
        const documentId = (0, uuid_1.v4)();
        const { data, error } = await supabaseAdmin_1.supabaseAdmin
            ...
            bucket: existing.bucket || STORAGE_BUCKET,
            ...
            .single();
        if (error || !data) {
            logger_1.logger.error('Failed to create final document record', { error });
    ...
    // GENERATE SIGNED URL
    // ===============================
    static async generateSignedUrl(filePath, expiresInSec = 60 * 60, bucket = STORAGE_BUCKET) {
        const { data, error } = await supabaseAdmin_1.supabaseAdmin.storage
            .from(bucket)
            .createSignedUrl(filePath, expiresInSec);
        if (error || !data) {
            logger_1.logger.error('Failed to create signed URL', { filePath, bucket, error });
            throw new Error('Failed to create signed url');
        }
        return data.signedUrl;
    }
    // Generate signed URL by document id (always read path from DB)
    static async generateSignedUrlFromDocument(documentId, expiresInSec = 60 * 60) {
        const { data: document, error } = await supabaseAdmin_1.supabaseAdmin
            .from('documents')
        ...
        if (error || !document?.file_path) {
            logger_1.logger.error('Document file_path not found', { documentId, error });
        ...
        const { data: signData, error: signError } = await supabaseAdmin_1.supabaseAdmin.storage
            .from(bucket)
            .createSignedUrl(filePath, expiresInSec);
        if (signError || !signData?.signedUrl) {
            logger_1.logger.error('Failed to create signed URL', { filePath, bucket, error: signError });
            throw new Error('Failed to create signed url');
        }
     ...
     */
    static async saveExtractionResult(documentId, extraction, source = 'python') {
        try {
            // Insert full JSON response into `document_extractions` as the source of truth
            const { data: insertData, error: insertError } = await supabaseAdmin_1.supabaseAdmin
            ...
            if (insertError) {
                logger_1.logger.warn('Failed to persist extraction to document_extractions', { documentId, error: insertError });
                return null;
            }
            logger_1.logger.info('Persisted extraction to document_extractions', { documentId, source });
            ...
            try {
                ...
                const { data: docData, error: docErr } = await supabaseAdmin_1.supabaseAdmin
                    .from('documents')
                    .select('request_id')
                    .eq('id', documentId)
                    .single();
                if (docErr || !docData) {
                    ...
                    if (docErr)
                        logger_1.logger.warn('Could not fetch document for propagation', { documentId, error: docErr });
                    return insertData;
                }
                const requestId = docData.request_id;
                if (!requestId)
                    return insertData;
                ...
                if (Array.isArray(extraction?.fields)) {
                    const blField = extraction.fields.find((x) => x && (x.key === 'bl_number' || x.key === 'BL'));
                ...
                if (!blNumber && extraction?.extraction && Array.isArray(extraction.extraction.fields)) {
                    const blField = extraction.extraction.fields.find((x) => x && (x.key === 'bl_number' || x.key === 'BL'));
                    if (blField && blField.value) {
                        blNumber = String(blField.value);
                ...
                if (!blNumber && extraction && typeof extraction.fields === 'object' && extraction.fields !== null && !Array.isArray(extraction.fields)) {
                    if (extraction.fields.bl_number)
                        blNumber = String(extraction.fields.bl_number);
                }
                const reqUpdate = {};
                if (blNumber)
                    reqUpdate.bl_number = blNumber;
                if (blConfidence != null)
                    reqUpdate.bl_confidence = blConfidence;
                try {
                    ...
                    if (normalized && typeof normalized === 'object') {
                        if (normalized.vessel)
                            reqUpdate.vessel = normalized.vessel;
                        if (normalized.voyage)
                            reqUpdate.voyage = normalized.voyage;
                        if (normalized.port_of_loading)
                            reqUpdate.port_of_loading = normalized.port_of_loading;
                        if (normalized.port_of_discharge)
                            reqUpdate.port_of_discharge = normalized.port_of_discharge;
                        if (normalized.shipped_on_board_date)
                            reqUpdate.shipped_on_board_date = normalized.shipped_on_board_date;
                        if (normalized.booking_no)
                            reqUpdate.booking_no = normalized.booking_no;
                    }
                }
                catch (e) {
                    logger_1.logger.warn('Failed to build request update payload from extraction', { e });
                }
                if (Object.keys(reqUpdate).length > 0) {
                    try {
                        await supabaseAdmin_1.supabaseAdmin.from('requests').update(reqUpdate).eq('id', requestId);
                        logger_1.logger.info('Propagated extraction fields to request', { requestId, reqUpdate });
                        if (blNumber) {
                            logger_1.logger.info('[BL-DETECTED] ' + `requestId=${requestId} bl=${blNumber} confidence=${blConfidence}`);
                        }
                    }
                    catch (e) {
                        logger_1.logger.warn('Failed to propagate extraction fields to request (columns may not exist)', { err: e, requestId, reqUpdate });
                    }
                }
                return insertData;
            }
            catch (e) {
                logger_1.logger.warn('Error while attempting to propagate extraction to request', { e });
                return insertData;
            }
        }
        catch (err) {
            logger_1.logger.warn('saveExtractionResult failed', { documentId, err });
            return null;
        }
    }
    // ===============================
    ...
    static async createAdminDocument(input) {
        ...
        logger_1.logger.info('Creating admin document', { requestId, fileName: file.originalname, type, uploadedBy });
        ...
        const { data: listData, error: listError } = await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).list(adminFolder);
        if (!listData || !Array.isArray(listData) || !listData.find(f => f.name === uniqueFileName)) {
            await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).remove([storedPath]).catch(() => null);
            logger_1.logger.error('Storage upload verification failed (admin)', { storagePath, uploadData, listError });
            throw new Error('Storage upload verification failed');
        }
        // Insert metadata
        const documentId = (0, uuid_1.v4)();
        ...
        if (dbError || !documentData) {
            // cleanup
            await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).remove([storedPath]);
            logger_1.logger.error('Failed to save admin document metadata', { dbError });
    ...
    static async createClientDocument(input) {
        ...
        logger_1.logger.info('Creating client document', { requestId, fileName: file.originalname, type, uploadedBy });
        // Validate file
        this.validateFile(file);
        const fileExtension = file.originalname.split('.').pop();
        const uniqueFileName = `${(0, uuid_1.v4)()}.${fileExtension}`;
        ...
        const { data: listData, error: listError } = await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).list(clientFolder);
        if (!listData || !Array.isArray(listData) || !listData.find(f => f.name === uniqueFileName)) {
            await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).remove([storedPath]).catch(() => null);
            logger_1.logger.error('Storage upload verification failed (client)', { storagePath, uploadData, listError });
            throw new Error('Storage upload verification failed');
        }
        const documentId = (0, uuid_1.v4)();
        const { data: documentData, error: dbError } = await supabaseAdmin_1.supabaseAdmin
        ...
        if (dbError || !documentData) {
            await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).remove([storedPath]);
            logger_1.logger.error('Failed to save client document metadata', { dbError });
    ...
    static async createFinalDocumentFromBuffer(params) {
        ...
        const { data: listData, error: listError } = await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).list(finalFolder);
        if (!listData || !Array.isArray(listData) || !listData.find(f => f.name === uniqueFileName)) {
            await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).remove([storedPath]).catch(() => null);
            logger_1.logger.error('Storage upload verification failed (final)', { storagePath, uploadData, listError });
            throw new Error('Storage upload verification failed');
        }
        const documentId = (0, uuid_1.v4)();
        const { data: documentData, error: dbError } = await supabaseAdmin_1.supabaseAdmin
        ...
        if (dbError || !documentData) {
            // cleanup
            await supabaseAdmin_1.supabaseAdmin.storage.from(STORAGE_BUCKET).remove([storedPath]);
            logger_1.logger.error('Failed to save final document metadata', { dbError });
    ...
    static async deleteDocument(documentId, userId, userRole) {
        ...
        const { error: storageError } = await supabaseAdmin_1.supabaseAdmin.storage
            .from(STORAGE_BUCKET)
            .remove([document.file_path]);
        if (storageError) {
            logger_1.logger.warn('Failed to delete file from storage', { storageError });
        }
        // Supprimer l'enregistrement en DB
        const { error: dbError } = await supabaseAdmin_1.supabaseAdmin
            .from('documents')
            .delete()
            .eq('id', documentId);
        if (dbError) {
            logger_1.logger.error('Failed to delete document from database', { dbError });

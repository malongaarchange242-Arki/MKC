#!/usr/bin/env node
const path = require('path');
require('dotenv').config({ path: path.resolve(__dirname, '../../.env') });
const { createClient } = require('@supabase/supabase-js');
const { randomUUID } = require('crypto');

const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY;
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('Missing SUPABASE_URL or SUPABASE key in environment');
  process.exit(2);
}

const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

(async () => {
  try {
    const requestId = process.argv[2] || randomUUID();
    const fileName = `testfile-${randomUUID()}.pdf`;
    const storagePath = `${requestId}/${fileName}`;
    // Minimal PDF header bytes (not a full PDF) but acceptable for test storage purposes
    const content = Buffer.from('%PDF-1.4\n%âãÏÓ\n1 0 obj\n<< /Type /Catalog >>\nendobj\n');

    console.log('Uploading to storage:', storagePath);
    const { data: upData, error: upErr } = await supabase.storage.from('documents').upload(storagePath, content, {
      contentType: 'application/pdf',
      upsert: false
    });

    if (upErr) {
      console.error('Upload error', upErr);
      process.exit(1);
    }

    console.log('Upload result:', upData);

    // Insert document row
    const docId = randomUUID();
      // Find an existing client user to use as client_id for the request
      const { data: users, error: usersErr } = await supabase.from('users').select('id').limit(1);
      if (usersErr) {
        console.error('Failed to fetch users', usersErr);
        process.exit(1);
      }

      if (!users || users.length === 0) {
        console.error('No users found in users table; cannot create request for test');
        process.exit(1);
      }

      const uploadedBy = users[0].id;

      // Ensure a request row exists for the requestId to satisfy FK
      const { data: reqData, error: reqErr } = await supabase.from('requests').insert({
        id: requestId,
        user_id: uploadedBy,
        type: 'FERI_AND_AD',
        status: 'CREATED'
      }).select().single();

      if (reqErr) {
        // if conflict (exists), ignore
        if (String(reqErr.message).includes('duplicate key')) {
          console.log('Request already exists, continuing');
        } else {
          console.error('Failed to create request row', reqErr);
          process.exit(1);
        }
      }

      const { data: docData, error: docErr } = await supabase.from('documents').insert({
      id: docId,
      request_id: requestId,
      file_name: fileName,
      file_path: storagePath,
      bucket: 'documents',
      file_size: content.length,
        mime_type: 'application/pdf',
      version: 1,
      uploaded_by: uploadedBy,
      type: 'test'
    }).select().single();

    if (docErr) {
      console.error('DB insert error', docErr);
      process.exit(1);
    }

    console.log('Inserted document row:', docData.id);

    // Immediately list and attempt signed URL
    console.log('Listing storage for requestId', requestId);
    const { data: listData, error: listErr } = await supabase.storage.from('documents').list(requestId);
    console.log('List result count =', Array.isArray(listData) ? listData.length : 0, 'err=', listErr);

    console.log('Attempting to create signed URL immediately');
    const { data: signData, error: signErr } = await supabase.storage.from('documents').createSignedUrl(storagePath, 60 * 60);
    console.log('Signed result:', { signData, signErr });

    process.exit(0);
  } catch (e) {
    console.error('Unexpected error', e);
    process.exit(1);
  }
})();

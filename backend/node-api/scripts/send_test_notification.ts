...
async function run() {
  ...
  const info = await transporter.sendMail({
    ...
    text: 'This is a test email sent from send_test_notification.ts to verify SMTP settings.'
  });

  console.log('Message sent:', info && (info as any).messageId ? (info as any).messageId : info);
}
